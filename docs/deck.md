...
7. ~~Whether the deployed upload page needs a slide.~~ **No, and this is the one place the deck earns its keep by not changing.** [#62](https://github.com/pedrodcsjostrom/adk-invoice-workshop/issues/62) rewrote 0:00 and 0:42 in the run of show: the deployed service now serves an upload page of ours and the way in is one command, so the collection ends on an upload against the attendee's own service rather than on a terminal printing a trace. Every minute of that is a live surface — a browser, a terminal, a page filling in — and a slide describing it would be a slide competing with the thing it describes. The five slides stand, the loop slide is still the only one that appears mid-hour, and the words that changed are in [`speaker-notes.md`](speaker-notes.md) under W4. Checked against the run of show and the notes together: the timings agree.
8. **Nothing here has been said out loud.** [#15](https://github.com/pedrodcsjostrom/adk-invoice-workshop/issues/15) is the test, and it must walk the cold-arrival path as well as the main one.
